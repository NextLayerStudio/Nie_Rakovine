...
export function PostCommentDrawer({
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    fetchPostCommentsAction(postId).then((res) => {
  ...
  return (
    <>
      {/* Backdrop — fixed over entire viewport */}
      <div
        className={cn(
      ...
      >
        ...
        {/* Input */}
        <div className="shrink-0 flex items-center gap-2 border-t border-brand-purple/10 px-4 py-3"
